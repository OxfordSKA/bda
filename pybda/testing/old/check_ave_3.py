...
def load_ms(ms):
    ...
    # tb.open(ms + '/POLARIZATION')
    # num_pols = tb.getcol('CORR_TYPE').shape[0]
    # tb.close()
    tb.open(ms)
    dtype = [('time', 'f8'),
             ('ctime', 'f8'),
...
def main():
    ...
    ms_bda_1 = os.path.join('vis', 'model_mstransform.ms')
    ...
    if check_fails:
        if check_type == 1:
            ...
            if vis_bda_1.shape[0] != vis_bda_2.shape[0]:
                print 'ERROR: row mismatch !'
                return

            print ''
            print 'Diffs: (%s - %s)' % (ms_bda_1, ms_bda_2)
            for t in vis_bda_1.dtype.descr:
                colname = t[0]
                print '  %10s : ' % (colname),
                print np.max(vis_bda_1[colname] - vis_bda_2[colname])
            print ''
