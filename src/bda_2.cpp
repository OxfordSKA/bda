...
int main(int argc, char** argv) {
    ...
    const ArrayColumn<Complex> col_model_data(ms, "MODEL_DATA");
    const ArrayColumn<Complex> col_corrected_data(ms, "CORRECTED_DATA");
    ...
    const Array<Complex> model_data_ = col_model_data.getColumn();
    const Array<Complex> corrected_data_ = col_corrected_data.getColumn();
    ...
    ArrayColumn<Complex> col_model_data_out(ms_out, "MODEL_DATA");
    ArrayColumn<Complex> col_corrected_data_out(ms_out, "CORRECTED_DATA");
    ...
    vector<Complex> ave_model_data_(num_baselines * num_pols, 0.0);
    vector<Complex> ave_corrected_data_(num_baselines * num_pols, 0.0);
    ...
    Complex* ave_model_data = &ave_model_data_[0];
    Complex* ave_corrected_data = &ave_corrected_data_[0];
        ...
        for (uInt b = 0, a1 = 0; a1 < num_antennas; ++a1) {
            for (uInt a2 = a1+1; a2 < num_antennas; ++a2, ++b) {
                ...
                for (int p = 0; p < num_pols; ++p) {
                    ...
                    ave_corrected_data[b * num_pols + p] += corrected_data[row * num_pols + p];
                }
                ...
                if (t == num_times - 1 || duvw[b] + b_duvw >= duvw_max
                    ...
                    for (int p = 0; p < num_pols; ++p) {
                        ...
                        double bda_model_re = ave_model_data[b * num_pols + p].real() / bda_count;
                        double bda_model_im = ave_model_data[b * num_pols + p].imag() / bda_count;
                        bda_model_data_p[p] = Complex(bda_model_re, bda_model_im);
                        double bda_corrected_re = ave_corrected_data[b * num_pols + p].real() / bda_count;
                        double bda_corrected_im = ave_corrected_data[b * num_pols + p].imag() / bda_count;
                        bda_corrected_data_p[p] = Complex(bda_corrected_re, bda_corrected_im);
                    }
                    ...
                    for (int p = 0; p < num_pols; ++p) {
                        ...
                        ave_model_data[b * num_pols + p].real() = 0.0;
                        ave_model_data[b * num_pols + p].imag() = 0.0;
                        ave_corrected_data[b * num_pols + p].real() = 0.0;
                        ave_corrected_data[b * num_pols + p].imag() = 0.0;
                    }
